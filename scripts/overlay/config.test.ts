...
function writeConfigFile(config: unknown) {
  const tempDirectory = mkdtempSync(join(tmpdir(), 'overlay-config-'));
    ...
    const { configDirectoryPath, cleanup } = writeConfigFile([
      {
        ...
        asset: {
          geoJson: 'data/broken-source/converted/overlay.geojson',
          metadata: 'data/broken-source/metadata/convert.json'
        },
        sync: {}
      }
    ]);
    ...
    const { configDirectoryPath, cleanup } = writeConfigFile([
      {
        ...
        asset: {
          geoJson: 'data/duplicate-source/converted/overlay.geojson',
          metadata: 'data/duplicate-source/metadata/convert.json'
        },
        sync: {
