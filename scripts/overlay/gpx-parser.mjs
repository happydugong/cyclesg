...
function buildFeature({
  ...
  return {
    ...
    properties: {
      ...
      routeId: `${overlayId}-${slugify(name) || featureIndex}-${geometryKind}-${featureIndex}`,
      overlayId,
      overlayName,
      sourceType: 'gpx',
      routeType: geometryKind === 'point' ? 'curated-poi' : 'curated-my-maps',
      routeSource: 'curated-my-maps',
      routeName: name,
      routeGroup: layerName ?? overlayName,
      routeLength: null,
      name,
      description,
      layerName,
...
export async function convertGpxFiles(filePaths, options = {}) {
  ...
  for (const filePath of filePaths) {
    ...
    const overlayRouteLength = Number(options.routeLengthByPath?.[filePath] ?? 0) || null;
    if (overlayRouteLength) {
      for (const feature of geoJson.features) {
        if (feature.properties.geometryKind === 'line') {
          feature.properties.routeLength = overlayRouteLength;
        }
      }
    }
